...
ROBOT_HALF_DIAGONAL = np.sqrt(ROBOT_HALF_HEIGHT_MM**2 + ROBOT_HALF_WIDTH_MM**2) - 50
...
def createLidarPlot():
    ...
    # map size/2
    p.x_range.start = -MAP_SIZE_MILLIMETERS / 4
    p.x_range.end = +MAP_SIZE_MILLIMETERS / 4

    p.y_range.start = -MAP_SIZE_MILLIMETERS / 4
    p.y_range.end = +MAP_SIZE_MILLIMETERS / 4
...
def createSlamPlot():
    ...
    p.patch(x='px', y='py', source=poseSource, fill_color="blue", line_color="black", alpha=1)
    # Configure the plot
...
def updateSlamPlot(message, datasources):
    ...
    Xs, Ys = makeRobotTriangle(x, y, thetaDeg-90)
